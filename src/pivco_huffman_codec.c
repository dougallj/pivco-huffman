...
/* ---------- Encode tree walk ---------- *
 *
 * DFS, POST-ORDER records (wire v0.6): the partition still runs at node
 * entry (it routes the ranks the recursion needs), and the K_right varint
 * is written there too, but the node's marker+bitmap record is emitted
 * AFTER the children's regions so the decoder consumes the stream
 * strictly forward (each bitmap sits exactly where its merge runs).
 * At each non-flat internal node, `ranks[0..n)` holds the surviving
 * leaves' in-order ranks; partition routes each by `rank >
 * split_rank[node]`, leaving the left half in place in `ranks[0..n_left)`
 * and compacting the right half into `tmp[0..n_right)`.  The recursion
 * descends left on `ranks`, right on `tmp`.  The bitmap is staged in a
 * stack buffer across the recursion (its final stream position depends on
 * the children's — FSE-variable — encoded sizes, so it can't be written
 * in place up front); ≤ bitmap_bytes(N)+64 per level, tree height ≤
 * PIVCO_MAX_CODE_LEN levels. */
                               ...
                               uint8_t *tmp)
{
    ...
    /* Non-flat internal node.  codec.c owns: K_right varint, FSE marker
     ...
     * partition the ranks.
     *
     * The bitmap is built into a stack staging buffer (+64 slack absorbs
     * the SIMD partitions' over-wide tail stores, which the pre-order
     * layout absorbed with later stream writes) and copied into the
     * stream after the children's regions. */
    ...
    case PIVCO_NODE_HALF_LEFT:
        n_right = prim_enc_partition_left(ranks, n, thr, bm_stage);        break;
    default:
    ...
    /* One varint split per recursion site (same slot set as the old
     * u16 K_right headers), consumed by the decoder at node entry so it
     * can size both children before their regions arrive. */
    if (kr_header_needed(table, node_id))
        wire_write_kr_varint(out_ptr, n_right);
 ...
 *
 * Wire v0.6 is post-order, so the input cursor is consumed strictly
 * forward AND each record is loaded exactly where it is used: the
 * K_right varint at node entry, the children's regions during their
 * recursion, the node's bitmap right before its merge.  (v0.5 read the
 * bitmap at entry and merged it after the recursion — a backwards
 * touch that kept the whole compressed block cache-resident.)  The
 * bm_scratch VLA for FSE-coded bitmaps is now also allocated after the
 * recursion, so its lifetime no longer spans the subtree.
 *
                                   ...
                                   uint8_t *scratch_top)
{
    ...
    case PIVCO_NODE_HALF_RIGHT: {
        /* If the right child is also a leaf, no recursion (and no
         * K_right varint on the wire): merge_cst_cst with prefill_sym
         * on the left and the right child's symbol on the right. */
        ...
        int K_right = wire_read_kr_varint(in_ptr);
    ...
    case PIVCO_NODE_HALF_LEFT: {
        ...
        int K_left = K - wire_read_kr_varint(in_ptr);
